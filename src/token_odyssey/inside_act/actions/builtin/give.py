...
def plan(context: ActionContext, raw: BaseActionIntent) -> ActionEffect:
    intent = cast(GiveIntent, raw)
    recipient_id = intent.target_ids[0]
    old = context.state.placements[intent.item_id].model_copy(deep=True)
    new = Placement(relation=PlacementRelation.ATTACHED, parent_id=recipient_id)
    return ActionEffect(
        data=GiveEventData(item_id=intent.item_id, recipient_id=recipient_id),
        mutations=[PlacementMutation(intent.item_id, old, new)],
        anchors=[actor_anchor(context.actor_id)],
        guaranteed_observer_ids=[recipient_id],
...
ACTION = ActionSpec(
    ...
    prompt_misuses=("公共位置的物品要先在更早的 action take",),
