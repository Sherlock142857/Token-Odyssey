...
from token_odyssey.agents.contracts import Participant
...
from token_odyssey.llm.contracts import LLMBackend, LLMRequest
...
def _openai_compatible(config: BackendConfig) -> LLMBackend:
    if config.api_key_env is not None:
        key = os.environ.get(config.api_key_env)
    else:
        assert config.api_key_file is not None
...
BACKEND_FACTORIES: dict[str, Callable[[BackendConfig], LLMBackend]] = {"openai_compatible": _openai_compatible}
...
def identity_for(scenario: Scenario, actor_id: str, campaign_context: str = "") -> LLMIdentity:
    ...
    return LLMIdentity(
        ...
        act_title=scenario.title,
        public_background=scenario.public_background,
        campaign_context=campaign_context,
        personality=brief.personality,
        private_goal=brief.private_goal,
        memories=brief.memories,
        known_entities=known,
    )


def build_scripted_participants(scenario: Scenario, registry: ActionRegistry) -> dict[str, Participant]:
    return {
        actor: ScriptedAgent(actor, [registry.parse_batch(raw) for raw in scenario.scripts.get(actor, ())], registry)
...
def build_participants(
    ...
    *,
    ...
    on_llm_request: Callable[[str, str, LLMRequest], None] | None = None,
    ...
    for actor, binding in cast.items():
        if binding.adapter == "scripted":
            result[actor] = ScriptedAgent(
                actor, [registry.parse_batch(raw) for raw in scenario.scripts.get(actor, ())], registry
            )
            ...
            result[actor] = LLMAgent(
                actor,
                LLMTranslator(registry, identity_for(scenario, actor, (identity_contexts or {}).get(actor, ""))),
                backends[profile.backend_id],
                profile,
