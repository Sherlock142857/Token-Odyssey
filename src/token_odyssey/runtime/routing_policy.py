...
DEFAULT_FACT_WEIGHTS = {
    ...
    "show": 1.0, "item_location": 0.4, "search": 0.7, "inspect": 0.7,
    "discovery": 1.5, "open": 1.0, "close": 0.8, "lock": 1.2,
    "unlock": 1.2, "move": 1.2, "install": 1.4, "operate": 1.0,
    "arrival": 1.2, "departure": 0.8, "mechanism_seen": 1.8,
    "mechanism_heard": 1.2,
}
...
class RoutingPolicy(FrozenModel):
    ...
    # Keep the legacy strength as the compatibility default. Acts can tune the
    # interaction router down without changing global code or fact semantics.
    ...
    def merge_weight_overrides(cls, value):
        if not isinstance(value, dict):
            return value
        data = dict(value)
        for field, defaults in (("fact_weights", DEFAULT_FACT_WEIGHTS),
