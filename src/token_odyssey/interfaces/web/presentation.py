    ...
    return [
        {
            ...
            "help": ACTION_HELP.get(kind, ""),
            "schema": registry.get(kind).intent_type.model_json_schema(),
        }
...
def event_text(event, labels):
    ...
    if event["source"] == "world":
        descriptions = [
            cue["fact"]["fields"]["description"] for cue in event["cues"] if "description" in cue["fact"]["fields"]
        ]
    ...
    if kind == "move":
        return (
            f"{actor}移动到{labels.get(data.get('destination_room_id'), data.get('destination_room_id', '另一房间'))}。"
        )
    if kind == "say":
