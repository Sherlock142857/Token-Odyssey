...
function canAct() { return !posting && state?.phase === "playing_act" && !state.act?.busy && state.act?.status === "waiting_for_input" && state.act.pending; }
...
function renderPlay(){
  ...
  const actNotes=[...new Set([state.interlude,state.act_background].filter(Boolean))];
  const nextReference=JSON.stringify([state.campaign_id,state.act_number,actNotes,state.act_goal,state.public_world,state.major_history]);
  if(referenceKey!==nextReference){referenceKey=nextReference;$("reference").innerHTML=`<section class="reference-card"><h3>本幕资料</h3>${actNotes.map((text)=>`<p>${esc(text)}</p>`).join("")||'<p class="muted">暂无额外背景。</p>'}${state.act_goal?`<p class="act-goal"><b>当前目标</b> · ${esc(state.act_goal)}</p>`:""}</section>
    <details class="reference-card"><summary>世界观与重大历史</summary><p>${esc(state.public_world||"")}</p>${state.major_history?.length?`<ul>${state.major_history.map((row)=>`<li>${esc(row)}</li>`).join("")}</ul>`:""}</details>`;}
...
function renderFields(){
  ...
  $("action-fields").innerHTML=fields.map((field)=>{
    ...
    if(field.endsWith("_ids"))return `<fieldset><legend>${fieldNames[field]}</legend>${options.map(([v,n])=>`<label class="check"><input type="checkbox" name="${field}" value="${esc(v)}">${esc(n)}</label>`).join("")||'<p class="muted">当前没有在场角色。</p>'}${field==="listener_ids"?'<p class="muted">不勾选表示对在场所有人公开发言。</p>':""}</fieldset>`;
    return `<label>${fieldNames[field]}<select name="${field}">${options.map(([v,n])=>option(v,n)).join("")||'<option value="">没有可选对象</option>'}</select></label>`;
  }).join("");
}
...
$("action-kind").addEventListener("change",renderFields); $("add-action").addEventListener("click",(e)=>{e.preventDefault();if(!canAct()||queue.length>=view().max_actions)return;const kind=$("action-kind").value,action={kind,amplitude:"normal"};for(const field of fieldsByKind[kind]){const controls=[...$("action-fields").querySelectorAll(`[name="${field}"]`)];action[field]=field.endsWith("_ids")?controls.filter((control)=>control.checked).map((control)=>control.value):controls[0]?.value;if(field==="passage_id"&&!action[field])delete action[field];}if(kind==="say"&&!action.content.trim())return showError("请填写台词。");if(kind==="show"&&!action.observer_ids.length)return showError("请选择展示对象。");queue.push(action);renderQueue();});
...
$("quick-wait").addEventListener("click",()=>{if(!canAct())return;queue=[];renderQueue();post("submit",{actor_id:state.act.pending.actor_id,request_id:state.act.pending.request_id,actions:[{kind:"wait"}],auto:true});});
