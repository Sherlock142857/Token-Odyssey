...
from token_odyssey.constants import DEFAULT_CAMPAIGN_LLM_TIMEOUT_SECONDS
from token_odyssey.llm.contracts import ChatMessage, ChatRole, LLMBackend, LLMExchange, LLMRequest, LLMResponse
...
class CampaignLLMService:
    ...
    def __init__(
        ...
        store: CampaignStore,
        *,
        timeout: float = DEFAULT_CAMPAIGN_LLM_TIMEOUT_SECONDS,
    ):
    ...
    def complete(self, profile_name: str, operation_id: str, actor_id: str, messages: list[ChatMessage]) -> LLMResponse:
        ...
        try:
            ...
            try:
                self.store.record(
                    "orchestration_exchanges",
                    LLMExchange(actor_id=actor_id, request_id=operation_id, request=request, error=type(exc).__name__),
                )
            finally:
                with self._active_lock:
        ...
        try:
            ...
            self.store.record(
                "orchestration_exchanges",
                LLMExchange(actor_id=actor_id, request_id=operation_id, request=request, response=response),
            )
            return response
        finally:
            with self._active_lock:
        ...
        for attempt in range(1, retries + 1):
            ...
            if operation_id not in self.state.director_completed_ops:
                prompt = (
                    ...
                    if attempt == 1
                    else (f"上一个 JSON 无法通过校验：{error}\n请按原契约修正，只输出完整 JSON 对象。" + contract)
                )
                if not any(
                    message.role == ChatRole.USER and marker in message.content
                    for message in self.state.director_messages
                ):
                    self.state.director_messages.append(ChatMessage(role=ChatRole.USER, content=f"{marker}\n{prompt}"))
                    self.save()
                response = self.llm.complete(self.profile_name, operation_id, "director", self.state.director_messages)
                self.state.director_messages.append(ChatMessage(role=ChatRole.ASSISTANT, content=response.content))
