...
默认 InteractionWeightedRouter 消费已提交事件的实际感知投影，结合定向回应、关注对象、刺激衰减和等待补偿选择角色。不会读取私人目标或用隐藏事件推断目击者。`routing.strategy` 还可选择 `shuffled`（逐轮随机置换）或 `weighted`（仅按 act 权重独立抽样）；数值、接口和选择证据见 [Router 说明](router.md)。
