...
run artifact schema v3 包括 manifest、规范化 Scenario、初始/最终 state、decisions、World Events、Observation outcomes、trace、token usage、LLM sessions、易读的 `prompt_flow.md` 和 transcript。Decision 使用 outcome 区分 `accepted`、`accepted_with_notice`、`truncated`、`rejected` 和 `fallback`，并单列 execution notices。Replay 只读取 schema v3；旧 v2 运行记录不兼容。Scenario 自身仍使用 schema v2。
